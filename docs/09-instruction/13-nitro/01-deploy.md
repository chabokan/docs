...
title: "استقرار اولیه"
icon: "faRocket"
description: "در این بخش می خواهیم آموزش نحوه استقرار اولیه هاست Nitro را به شما آموزش دهیم"
---

# استقرار اولیه Nitro
---

در این بخش می خواهیم آموزش نحوه استقرار اولیه Nitro را به شما آموزش دهیم. Nitro یک فریمورک مبتنی بر Javascript است که
برای
...
![استقرار Nitro در چابکان](https://s1.chabokan.net/docs/images/nodejs-docs-1.png)

پس از چند دقیقه، سرویس شما با موفقیت ساخته می‌شود. حال می‌توانید با آپلود سورس‌کد برنامه خود، `Nitro` را روی سرویس‌های
...
برای استقرار `Nitro` روی سرویس `NodeJS` حتما مقدار `scripts.start` را در فایل `package.json` مانند زیر تنظیم
کنید:
...
فریمورک `Nitro` به صورت پیشفرض روی پورت `3000` فعال می‌شود و سرویس میزبانی آسان NodeJS چابکان نیز به صورت پیشفرض روی
همین
پورت فعال است. اما برای تغییر پورت پیشفرض شما‌می‌توانید این [مستند](/features/ports) را مطالعه بفرمائید.

![استقرار Nitro در چابکان](https://s1.chabokan.net/docs/images/nitro-port.png)
...
:::caution دقت کنید
برای استفاده از `Nitro` در هاست `NodeJS` باید حداقل از نسخه `18` استفاده کنید.
:::
